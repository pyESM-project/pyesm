...
.. admonition:: Problem statement
  ...
  .. math::
    ...
    \hline
      c_0 \; [\text{€/u}]      & 1.0    & 2.2    \\
      c_s \; [\text{€/u}^2]    & 0.005 & 0.01 \\
      e \; [\text{kWh/u}]      & 1.7    & 3.5    \\
      m \; [\text{kg/u}]       & 0.5    & 1.2
    \end{array}

  Positive slopes :math:`c_{s,j} > 0` represent **economies of scale**: the more 
  a product is sold, the higher its unit margin becomes. A linear relationship 
  between profit and production level is assumed, with :math:`c_{0,j}` representing 
  the unit profit at zero production and :math:`c_{s,j}` the increase in unit profit 
  per additional unit produced. Finally, the energy and material endowments remain:
...
.. list-table:: Results from ``database.db`` | ``profit`` table (converged unit profits)
   :header-rows: 1
   :widths: 15 15 25 25
...
- In all scenarios, the optimal solution concentrates production entirely on
  :math:`product_1` (:math:`x_2 = 0`). This differs from the base tutorial,
  where :math:`product_2` dominated energy-binding scenarios. The **economies
  of scale** (positive :math:`c_s`) increasingly reward high production volumes,
  making :math:`product_1`, with the lower energy and material intensity, the
  dominant choice once its effective margin rises above that of :math:`product_2`.
- The binding constraint rotates with resource availability: energy limits
  output in the low-energy scenarios and the high-energy/high-material case,
  while material becomes the binding constraint in the high-energy/low-material
  scenario.
- The converged unit profits in the ``profit`` table reflect the effective
  margin at the optimal production level. For :math:`product_1` in
  energy-binding scenarios: :math:`c_1 = 1.0 + 0.005 \times x_1`. For
  example, in the ``high``/``high`` scenario: 
  :math:`c_1 = 1.0 + 0.005 \times 176.471 = 1.882`.
- :math:`c_2` values in the low-energy scenarios (2.629) reflect the unit
  profit of :math:`product_2` evaluated at the production level it had during
  the first Gauss–Seidel iteration, before the LP shifted entirely to
  :math:`product_1`. Since :math:`x_2 = 0` at convergence, :math:`c_2` does
  not directly influence the final objective but is still stored in the database
  as the last computed value.
